...
describe("TaskGraphProcessor", () => {
  ...
  describe("type 분류", () => {
    const cases: [string, string, string][] = [
      ...
      ["Trace and explain the order in which data passes through the router", "analyze", "trace and explain flow"],
      ...
      ["Organize the changes made and command results in the work note", "document", "organize work note"],
      ["Organize the duplicate validation logic into a shared service", "modify",   "organize code → modify idiom"],
      ["Organize the flow of how error handling is done",              "analyze",  "organize flow → analyze idiom"],
      ["Organize the request pipeline to map all entry points",        "analyze",  "organize pipeline → analyze idiom"],
      ["check whether the payment flow is intact",                     "validate", "check whether → validate"],
      ["check that all edge cases are handled",                        "validate", "check that → validate"],
      ["check if the same error handling logic is duplicated",         "analyze",  "check if duplicated → analyze"],
      ["check whether any response conversion logic is redundant",     "analyze",  "check whether redundant → analyze"],
      ["catalog all API endpoints in the project",                     "explore",  "catalog → explore"],
      ["enumerate all routes registered in the router",                "explore",  "enumerate → explore"],
      ["audit the error handling code",                                "analyze",  "audit → analyze"],
      ["identify issues in the authentication module",                 "analyze",  "identify issues → analyze"],
      ["detect redundant error handlers across the codebase",          "analyze",  "detect redundant → analyze"],
      ["extract the common error handling into a shared utility",      "modify",   "extract → modify refactoring"],
      ["consolidate the duplicated logic into one shared service",     "modify",   "consolidate → modify refactoring"],
      ["centralize the logging configuration in one module",           "modify",   "centralize → modify refactoring"],
      ["restructure the API layer to separate concerns",               "modify",   "restructure → modify refactoring"],
      ["decouple the UI components from the data layer",               "modify",   "decouple → modify refactoring"],
      ["streamline the request handling pipeline",                     "modify",   "streamline → modify refactoring"],
