...
import { EmbeddingIndexer } from "../../../../../src/core/rag/embedding-indexer.js";
...
describe("EmbeddingIndexer", () => {
  ...
  it("세션 완료 후 raw_input을 prompt 벡터로 인덱싱한다", async () => {
    ...
    expect(embedder.embed).toHaveBeenCalledWith(expect.stringContaining("Find the auth module"));
    expect(store.upsert).toHaveBeenCalledWith(
      expect.stringContaining("prompt::sess-abc"),
      expect.any(Float32Array),
      expect.objectContaining({ kind: "prompt", session_id: "sess-abc" }),
    );
  });

  it("완료된 task의 title(raw_output)을 task 벡터로 인덱싱한다", async () => {
    await indexer.indexSession(makeSession() as any);
  ...
  it("raw_output을 output 벡터로 인덱싱한다", async () => {
    ...
    expect(store.upsert).toHaveBeenCalledWith(
      ...
      expect.any(Float32Array),
      expect.objectContaining({ kind: "output", session_id: "sess-abc", task_id: "t1" }),
    );
  });
  ...
  it("raw_input이 없는 세션은 prompt 인덱싱을 건너뛴다", async () => {
    ...
    const promptCalls = (store.upsert as ReturnType<typeof vi.fn>).mock.calls.filter(
  ...
  it("success=false인 task는 인덱싱하지 않는다", async () => {
    const session = makeSession({
      task_results: {
        t1: { task_id: "t1", success: false, raw_output: "failed output", summary: "fail", input_hash: "h1" },
      },
    });
    await indexer.indexSession(session as any);

    const taskCalls = (store.upsert as ReturnType<typeof vi.fn>).mock.calls.filter(
