...
async function createLoadedRuntime(
	...
	try {
		...
		logger.warn(
			`node-llama-cpp GPU startup failed, retrying with CPU only: ${message}`,
		);
...
function convertMessagesToChatHistory(messages: LlmMessage[]): {
	chatHistory: ChatHistoryItem[];
	prompt: string;
} {
...
export async function completeChatWithNodeLlamaCpp(
	...
	const { chatHistory, prompt } = convertMessagesToChatHistory(
		request.messages,
	);
	const isQwenReasoningArchitecture =
