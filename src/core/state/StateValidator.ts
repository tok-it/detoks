...
export class StateValidator {
  ...
  private static validateBusinessRules(state: SessionState): void {
    ...
    if (state.current_task_id && state.completed_task_ids.includes(state.current_task_id)) {
      throw new Error(`Invalid State: current_task_id [${state.current_task_id}] is already marked as completed.`);
    }
    ...
    if (!state.shared_context || typeof state.shared_context !== 'object' || Object.keys(state.shared_context).length === 0) {
      throw new Error('Invalid State: shared_context must be a non-empty object (Two-Tier structure requirement).');
    }

    // 규칙 3: Task 결과 일관성 확인
    for (const taskId of state.completed_task_ids) {
      if (!state.task_results[taskId]) {
        throw new Error(`Invalid State: Task [${taskId}] is completed but has no result in task_results.`);
      }
    }
  }
   ...
   */
  private static validateSerializable(state: any): void {
    try {
      const serialized = JSON.stringify(state);
      JSON.parse(serialized);
    } catch (error) {
      throw new Error(`Invalid State: State is not serializable or contains circular references. ${error}`);
    }
  }
