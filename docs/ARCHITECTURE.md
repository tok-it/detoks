...
↓  
User Output
<!-- 한국어 설명: 입력은 프롬프트 정제, 요청 분석, 작업 그래프 생성, 컨텍스트 최적화 단계를 거쳐 LLM CLI로 전달되고, 이후 결과 후처리와 상태 저장을 거쳐 사용자에게 반환됩니다. -->
...
- Prompt processing
- Task analysis
<!-- 한국어 설명: LLM Layer는 프롬프트를 가공하고 요청을 해석해 작업 단위로 분석하는 역할을 합니다. -->
