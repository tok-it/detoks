...
**학술 근거**: Manning et al. (2008) *"Introduction to Information Retrieval"*, Cambridge University Press
> 규칙 기반 분류는 훈련 데이터 없이 높은 정밀도를 달성할 수 있으며,  
> **도메인이 명확하고 어휘가 제한된** 환경에서 통계적 분류기와 경쟁 가능한 성능을 보인다.
...
**학술 근거**: Ullman (1975) *"NP-complete scheduling problems"*, Journal of Computer and System Sciences
> 워크플로우의 의존성 관계가 도메인 지식으로 사전 정의 가능할 때,  
> 정적 전이 테이블은 동적 예측보다 안정적이고 검증 가능한 스케줄링을 보장한다.
...
(Ullman, 1975의 정적 스케줄링 원칙 기반)을 채택했다. LLM 동적 예측은
...
| Ullman, J.D. | 1975 | NP-complete scheduling problems | *Journal of Computer and System Sciences* |
